import { pgTable, varchar, timestamp, text, uuid, boolean, jsonb, integer } from 'drizzle-orm/pg-core';
...
export const ticketPanels = pgTable('ticket_panels', {
  ...
  ticketNameFormat: varchar('ticket_name_format', { length: 100 }).default('ticket-{number}').notNull(),
...
export const tickets = pgTable('tickets', {
  ...
  closedBy: varchar('closed_by', { length: 20 }).references(() => users.id, { onDelete: 'set null' }),
  closedReason: text('closed_reason'),
  closedAt: timestamp('closed_at'),
  lockedBy: varchar('locked_by', { length: 20 }).references(() => users.id, { onDelete: 'set null' }),
  lockedAt: timestamp('locked_at'),
  frozenBy: varchar('frozen_by', { length: 20 }).references(() => users.id, { onDelete: 'set null' }),
