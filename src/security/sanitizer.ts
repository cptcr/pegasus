...
export class Sanitizer {
  ...
  static sanitizeCodeBlock(code: string, language?: string): string {
    ...
    const validLanguages = ['js', 'javascript', 'ts', 'typescript', 'py', 'python', 
                          'java', 'c', 'cpp', 'cs', 'php', 'rb', 'go', 'rs', 
                          'swift', 'kotlin', 'scala', 'r', 'sql', 'bash', 'sh',
                          'powershell', 'json', 'yaml', 'xml', 'html', 'css',
                          'markdown', 'md', 'diff', 'ini', 'toml'];
    
    const lang = language && validLanguages.includes(language.toLowerCase()) 
