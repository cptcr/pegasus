...
router.get('/:guildId/moderation', async (req: Request, res: Response) => {
  ...
  try {
    ...
    stats.forEach((stat: any) => {
      ...
      switch (stat.type) {
        case 'warn': moderationStats.totalWarnings = count; break;
        case 'ban': moderationStats.totalBans = count; break;
        case 'mute': moderationStats.totalMutes = count; break;
        case 'kick': moderationStats.totalKicks = count; break;
      }
    });
...
router.get('/:guildId/giveaways', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const entryCounts = await db
      ...
      .where(sql`${giveawayEntries.giveawayId} IN ${sql.raw(`(${activeGiveaways.map((g: any) => `'${g.giveawayId}'`).join(',')})`)}`)
...
router.get('/:guildId/members', async (req: Request, res: Response): Promise<Response | void> => {
  ...
  try {
    ...
    const response = {
      members: members.first(20)?.map(m => ({
        id: m.id,
...
router.get('/:guildId/notifications', async (req: Request, res: Response): Promise<Response | void> => {
  const { guildId } = req.params;
  const db = getDatabase();
