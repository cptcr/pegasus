import { pgTable, varchar, bigint, timestamp, text, boolean, integer, jsonb, primaryKey, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
// Shop items
export const economyShopItems = pgTable('economy_shop_items', {
  id: varchar('id', { length: 255 }).primaryKey().notNull().$defaultFn(() => crypto.randomUUID()),
  guildId: varchar('guild_id', { length: 255 }).notNull(),
  ...
  uniqueNamePerGuild: uniqueIndex('economy_shop_items_guild_name_unique').on(table.guildId, table.name),
}));

// User purchased items
export const economyUserItems = pgTable('economy_user_items', {
  id: varchar('id', { length: 255 }).primaryKey().notNull().$defaultFn(() => crypto.randomUUID()),
  userId: varchar('user_id', { length: 255 }).notNull(),
  guildId: varchar('guild_id', { length: 255 }).notNull(),
  itemId: varchar('item_id', { length: 255 }).notNull().references(() => economyShopItems.id, { onDelete: 'cascade' }),
...
export const economyCooldowns = pgTable('economy_cooldowns', {
  userId: varchar('user_id', { length: 255 }).notNull(),
  guildId: varchar('guild_id', { length: 255 }).notNull(),
...
export const economyGamblingStats = pgTable('economy_gambling_stats', {
  userId: varchar('user_id', { length: 255 }).notNull(),
  guildId: varchar('guild_id', { length: 255 }).notNull(),
